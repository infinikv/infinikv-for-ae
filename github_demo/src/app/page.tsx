...
import { t, useLocale, LOCALE_STORAGE_KEY } from "@/lib/i18n";

// On the very first home-page visit of each browser session, reset locale to English.
// This runs before useLocale reads localStorage, so there is no language flash.
// Subsequent navigation and manual language switches are unaffected.
if (typeof window !== "undefined") {
  const sessionKey = "infinikv_home_init";
...
function StatNumber({ value, unit, label }: { value: string; unit: string; label: string }) {
  return (
    <div className="text-center">
      <div className="flex items-baseline justify-center space-x-1">
        <span className="text-5xl md:text-6xl font-mono font-black text-sky-600 tracking-tighter">{value}</span>
        <span className="text-xl text-sky-400 font-mono font-semibold">{unit}</span>
      </div>
      <div className="text-base text-gray-600 mt-3 tracking-wide font-semibold leading-snug">{label}</div>
    </div>
  );
}

function TechCard({ icon: Icon, title, desc, iconColor, iconBg }: { icon: any; title: string; desc: string; iconColor: string; iconBg: string }) {
  return (
...
function ScenarioCard({ href, title, accent, tag }: { href: string; title: string; accent: string; tag: string }) {
  return (
    <Link href={href} className="block group h-full">
      <div className="h-full min-h-[120px] bg-white border border-gray-200 rounded-2xl p-4 card-hover relative overflow-hidden shadow-sm flex flex-col">
        ...
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs font-mono font-bold px-2 py-0.5 rounded-md bg-gray-100 border border-gray-200 text-gray-500">{tag}</span>
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-24 text-center relative z-10">
          <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={0}>
            <div className="inline-flex items-center space-x-2.5 bg-white/80 backdrop-blur border border-gray-200/80 px-6 py-2.5 rounded-full mb-10 shadow-sm">
              ...
              <span className="text-base font-semibold text-gray-700 tracking-wide">{tr("home.badge")}</span>
            </div>
          </motion.div>
          ...
          <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={1}
            ...
            <h1 className="text-6xl sm:text-7xl md:text-8xl font-black tracking-tight">
              <span className="gradient-text-hero" style={{ paddingRight: "0.05em" }}>InfiniKV</span>
            </h1>
          </motion.div>
      ...
      {/* Key Numbers */}
      <section className="border-y border-gray-100 bg-gradient-to-b from-gray-50/80 to-white py-20">
        <div className="max-w-6xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-10">
          <StatNumber value="2-3.5" unit="x" label={tr("home.stat.ttft")} />
          <StatNumber value="98.3" unit="%" label={tr("home.stat.hit")} />
          <StatNumber value="66" unit="%" label={tr("home.stat.cost")} />
      ...
      <section id="scenarios" className="max-w-6xl mx-auto px-4 pb-24">
        <div className="text-center mb-14">
          <h2 className="text-4xl font-black text-gray-900 mb-4 tracking-tight">{tr("home.scenarios.title")}</h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 items-stretch">
          <ScenarioCard href="/demo/drone-delivery" tag={tr("home.scenario.01")}
            title={tr("home.scenario.single")}
            accent="from-emerald-400 to-teal-500" />
          <ScenarioCard href="/demo/drone-fleet" tag={tr("home.scenario.02")}
            title={tr("home.scenario.multi")}
            accent="from-rose-400 to-pink-500" />

          <ScenarioCard href="/demo/stress-test" tag={tr("home.scenario.03")}
      ...
      <section className="max-w-6xl mx-auto px-4 pb-24">
        <div className="bg-gradient-to-br from-gray-50 to-slate-50 border border-gray-200 rounded-3xl p-8 md:p-14">
          <h2 className="text-3xl font-black text-gray-900 mb-10 text-center tracking-tight">{tr("home.storage.title")}</h2>
          <div className="space-y-4 max-w-xl mx-auto">
            {[
              { label: "GPU HBM", bw: "2-4 TB/s", cap: "80 GB", cost: "$$$$$", color: "bg-purple-500", width: "w-full" },
              { label: "CPU DRAM", bw: "50 GB/s", cap: "512 GB", cost: "$$$", color: "bg-amber-500", width: "w-5/6" },
              { label: "NVMe SSD (InfiniKV)", bw: "25.9 GB/s", cap: "14+ TB", cost: "$", color: "bg-sky-500", width: "w-4/6" },
            ].map((tier, i) => (
              <div key={i} className="flex items-center space-x-4">
                <div className={`${tier.width} ${tier.color} rounded-xl px-5 py-4 flex items-center justify-between shadow-sm`}>
            ...
            <div className="text-center pt-4">
              <p className="text-sm text-gray-500">
                {tr("home.storage.note.a")}<span className="text-sky-600 font-medium">{tr("home.storage.note.b")}</span>{tr("home.storage.note.c")}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            <div className="p-10 border-b md:border-b-0 md:border-r border-gray-200">
              ...
              <ul className="space-y-4 text-[15px] text-gray-600">
                <li className="flex items-start space-x-3"><Zap className="w-5 h-5 text-sky-500 mt-0.5 flex-shrink-0" /><span>{tr("home.compare.ssd.1")}</span></li>
                <li className="flex items-start space-x-3"><Zap className="w-5 h-5 text-sky-500 mt-0.5 flex-shrink-0" /><span>{tr("home.compare.ssd.2")}</span></li>
                <li className="flex items-start space-x-3"><Zap className="w-5 h-5 text-sky-500 mt-0.5 flex-shrink-0" /><span>{tr("home.compare.ssd.3")}</span></li>
                <li className="flex items-start space-x-3"><Zap className="w-5 h-5 text-sky-500 mt-0.5 flex-shrink-0" /><span>{tr("home.compare.ssd.4")}</span></li>
                <li className="flex items-start space-x-3"><Zap className="w-5 h-5 text-sky-500 mt-0.5 flex-shrink-0" /><span>{tr("home.compare.ssd.5")}</span></li>
              </ul>
            </div>
            <div className="p-10">
              <div className="flex items-center space-x-3 mb-7">
              ...
              <ul className="space-y-4 text-[15px] text-gray-600">
                <li className="flex items-start space-x-3"><Server className="w-5 h-5 text-amber-500 mt-0.5 flex-shrink-0" /><span>{tr("home.compare.dram.1")}</span></li>
                <li className="flex items-start space-x-3"><Server className="w-5 h-5 text-amber-500 mt-0.5 flex-shrink-0" /><span>{tr("home.compare.dram.2")}</span></li>
                <li className="flex items-start space-x-3"><Server className="w-5 h-5 text-amber-500 mt-0.5 flex-shrink-0" /><span>{tr("home.compare.dram.3")}</span></li>
                <li className="flex items-start space-x-3"><Server className="w-5 h-5 text-amber-500 mt-0.5 flex-shrink-0" /><span>{tr("home.compare.dram.4")}</span></li>
                <li className="flex items-start space-x-3"><Server className="w-5 h-5 text-amber-500 mt-0.5 flex-shrink-0" /><span>{tr("home.compare.dram.5")}</span></li>
              </ul>
            </div>
          </div>
          <div className="border-t border-gray-200 bg-gradient-to-r from-gray-50 to-slate-50 px-10 py-5 text-center">
            <p className="text-base text-gray-500 leading-relaxed">
              {tr("home.compare.summary.a")}<span className="text-sky-600 font-semibold">{tr("home.compare.summary.latency")}</span>
              {tr("home.compare.summary.b")}<span className="text-sky-600 font-semibold">{tr("home.compare.summary.capacity")}</span>
              {tr("home.compare.summary.c")}<span className="text-amber-600 font-semibold">{tr("home.compare.summary.cost")}</span>
            </p>
          </div>
        </div>
      </section>
