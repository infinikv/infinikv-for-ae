#!/usr/bin/bash
#
# This test script runs integration tests for the InfiniKV integration with vLLM.
# A infinikv/vllm-openai container image is built by this script from the InfiniKV code base 
...
#
# Note: The script should be run from the InfiniKV code base root.
...
build_infinikv_vllmopenai_image() {
...
run_infinikv_vllmopenai_container() {
    if [ -z "$HF_TOKEN" ]; then
        CID=$(docker run -d --runtime nvidia --gpus all \
            --env "INFINIKV_CHUNK_SIZE=256" \
            --env "INFINIKV_LOCAL_CPU=True" \
            --env "INFINIKV_MAX_LOCAL_CPU_SIZE=5" \
            --volume ~/.cache/huggingface:/root/.cache/huggingface \
            --network host \
            'infinikv/vllm-openai:build-latest' \
            'meta-llama/Llama-3.1-8B-Instruct' --kv-transfer-config \
            '{"kv_connector":"InfiniKVConnectorV1","kv_role":"kv_both"}')
    else
        CID=$(docker run -d --runtime nvidia --gpus all \
...
test_vllmopenai_server_with_infinikv_integrated() {
...
# test that can inference model using vLLM OpenAI API (infinikv integrated)
test_vllmopenai_server_with_infinikv_integrated
