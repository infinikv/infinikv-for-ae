# InfiniKV MMLU Testing Suite

## Overview
Tests InfiniKV KV transfer correctness vs vLLM baseline using MMLU benchmark.
...
./deploy-2-infinikv.sh "meta-llama/Llama-3.1-8B"  
...
Pipeline tests both models with vLLM baseline and InfiniKV KV transfer (4 total tests).

## Files
- **`deploy-1-vllm.sh`**: Single vLLM engine (port 8000) - for baseline
- **`deploy-2-infinikv.sh`**: Dual InfiniKV engines (ports 8000/8001) + Redis - for KV transfer
- **`1-mmlu.py`**: MMLU test on single vLLM engine (baseline)
- **`2-mmlu.py`**: MMLU test on dual InfiniKV engines (KV transfer)
