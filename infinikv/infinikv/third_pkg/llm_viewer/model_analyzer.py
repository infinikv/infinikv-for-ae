...
class ModelAnalyzer:
    def __init__(self, model_id, hardware, system_info: system_info, config_file=None, source="huggingface"):
        ...
        if source == "huggingface":
            ...
            relative_path = os.path.join(os.path.dirname(__file__), f"model_params/{source}.py")
            if not os.path.exists(relative_path):
                raise Exception(f"{relative_path} is not found")
            try:
            # Dynamically import the model parameters based on the source
                module = importlib.import_module(f".model_params.{source}", package="infinikv.third_pkg.llm_viewer")
        ...
        self.config = importlib.import_module(f".{config_file.replace('/', '.').replace('.py', '')}", package="infinikv.third_pkg.llm_viewer")
