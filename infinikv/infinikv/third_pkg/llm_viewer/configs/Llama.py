...
def get_num_attention_heads(model_params):
    return model_params["Llama-3.1-8B-Instruct"].num_attention_heads

def get_hidden_size(model_params):
    return model_params["Llama-3.1-8B-Instruct"].hidden_size

def get_num_key_value_heads(model_params):
    return model_params["Llama-3.1-8B-Instruct"].num_key_value_heads

def get_norm_layers(model_params):
    return ["attn_norm", "mlp_norm"]

def get_num_hidden_layers(model_params):
    return model_params["Llama-3.1-8B-Instruct"].num_hidden_layers

def get_intermediate_size(model_params):
    return model_params["Llama-3.1-8B-Instruct"].intermediate_size

def get_vocab_size(model_params):
    return model_params["Llama-3.1-8B-Instruct"].vocab_size
