...
def _infinikv_nvtx_annotate(func, domain="infinikv"):
    """Decorator for applying nvtx annotations to methods in infinikv."""
    return annotate(
...
def parse_hash_to_id(hash_string: str) -> int:
    """
    convert hash string to id
    Args:
    ...
    Returns:
        int: unique id
    """
    # use SHA256 hash algorithm to ensure uniqueness
    m = hashlib.sha256()
    m.update(hash_string.encode('utf-8'))
    hash_hex = m.hexdigest()
    
    # 取前8个字符作为整数ID，确保在合理范围内
    # 使用32位整数，避免溢出
    hash_int = int(hash_hex[:8], 16)
    
    # 确保ID为正数且在合理范围内
    # 使用模运算确保ID在0到max_num_local_file范围内
    return hash_int & 0x7FFFFFFF  # 确保是正数且不超过32位
