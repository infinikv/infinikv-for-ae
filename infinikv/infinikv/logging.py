...
def build_format(color):
    ...
    underline = "\x1b[3m"
    return (
        f"{color}[%(asctime)s] InfiniKV %(levelname)s:{reset} %(message)s "
...
def get_log_level() -> int:
    """
    Try to read INFINIKV_LOG_LEVEL from environment variables.
    Could be:
    - DEBUG
    - INFO
    - WARNING
    ...
    """
    log_level = os.getenv("INFINIKV_LOG_LEVEL", "INFO").upper()
...
#
# logging.basicConfig(
#    format="\033[33m%(levelname)s InfiniKV: \033[0m%(message)s "
