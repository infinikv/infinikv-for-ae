...
from infinikv.utils import thread_safe
...
class InfiniKVStatsMonitor:
    ...
    @thread_safe
    def get_stats_and_clear(self) -> InfiniKVStats:
        """
    ...
    @staticmethod
    def GetOrCreate() -> "InfiniKVStatsMonitor":
        if InfiniKVStatsMonitor._instance is None:
            InfiniKVStatsMonitor._instance = InfiniKVStatsMonitor()
        return InfiniKVStatsMonitor._instance

    @staticmethod
    def DestroyInstance():
        InfiniKVStatsMonitor._instance = None
...
class PrometheusLogger:
    ...
    def __init__(self, metadata: InfiniKVEngineMetadata):
        # Ensure PROMETHEUS_MULTIPROC_DIR is set before any metric registration
        if "PROMETHEUS_MULTIPROC_DIR" not in os.environ:
            default_dir = "/tmp/infinikv_prometheus"
        ...
        self.counter_num_retrieve_requests = self._counter_cls(
            name="infinikv:num_retrieve_requests",
            documentation="Total number of retrieve requests sent to infinikv",
            labelnames=labelnames,
        )

        self.counter_num_store_requests = self._counter_cls(
            name="infinikv:num_store_requests",
            documentation="Total number of store requests sent to infinikv",
            labelnames=labelnames,
        )

        self.counter_num_requested_tokens = self._counter_cls(
            name="infinikv:num_requested_tokens",
            documentation="Total number of tokens requested from infinikv",
            labelnames=labelnames,
        )

        self.counter_num_hit_tokens = self._counter_cls(
            name="infinikv:num_hit_tokens",
            documentation="Total number of tokens hit in infinikv",
            labelnames=labelnames,
        )

        self.counter_num_remote_read_requests = self._counter_cls(
            name="infinikv:num_remote_read_requests",
            documentation="Total number of requests read from "
            "remote backends in infinikv",
            labelnames=labelnames,
        )

        self.counter_num_remote_read_bytes = self._counter_cls(
            name="infinikv:num_remote_read_bytes",
            documentation="Total number of bytes read from remote backends in infinikv",
            labelnames=labelnames,
        )

        self.counter_num_remote_write_requests = self._counter_cls(
            name="infinikv:num_remote_write_requests",
        ...
        self.counter_num_remote_write_bytes = self._counter_cls(
            name="infinikv:num_remote_write_bytes",
            documentation="Total number of bytes write to remote backends in infinikv",
            labelnames=labelnames,
        )

        self.gauge_cache_hit_rate = self._gauge_cls(
            name="infinikv:cache_hit_rate",
            documentation="Cache hit rate of infinikv since last log",
            labelnames=labelnames,
            multiprocess_mode="livemostrecent",
        )

        self.gauge_local_cache_usage = self._gauge_cls(
            name="infinikv:local_cache_usage",
            documentation="Local cache usage (bytes) of infinikv",
            labelnames=labelnames,
            multiprocess_mode="sum",
        )

        self.gauge_remote_cache_usage = self._gauge_cls(
            name="infinikv:remote_cache_usage",
            documentation="Remote cache usage (bytes) of infinikv",
            labelnames=labelnames,
            multiprocess_mode="sum",
        )

        self.gauge_local_storage_usage = self._gauge_cls(
            name="infinikv:local_storage_usage",
            documentation="Local storage usage (bytes) of infinikv",
            labelnames=labelnames,
            multiprocess_mode="sum",
        )
        ...
        self.histogram_time_to_retrieve = self._histogram_cls(
            name="infinikv:time_to_retrieve",
            documentation="Time to retrieve from infinikv (seconds)",
            labelnames=labelnames,
        ...
        self.histogram_time_to_store = self._histogram_cls(
            name="infinikv:time_to_store",
            documentation="Time to store to infinikv (seconds)",
            labelnames=labelnames,
        ...
        self.histogram_retrieve_speed = self._histogram_cls(
            name="infinikv:retrieve_speed",
            documentation="Retrieve speed of infinikv (tokens per second)",
            labelnames=labelnames,
        ...
        self.histogram_store_speed = self._histogram_cls(
            name="infinikv:store_speed",
            documentation="Store speed of infinikv (tokens per second)",
            labelnames=labelnames,
        ...
        self.histogram_remote_time_to_get = self._histogram_cls(
            name="infinikv:remote_time_to_get",
        ...
        self.histogram_remote_time_to_put = self._histogram_cls(
            name="infinikv:remote_time_to_put",
        ...
        self.histogram_remote_time_to_get_sync = self._histogram_cls(
            name="infinikv:remote_time_to_get_sync",
    ...
    def log_prometheus(self, stats: InfiniKVStats):
        self._log_counter(
    ...
    @staticmethod
    def _metadata_to_labels(metadata: InfiniKVEngineMetadata):
        return {
    ...
    @staticmethod
    def GetOrCreate(metadata: InfiniKVEngineMetadata) -> "PrometheusLogger":
...
class InfiniKVStatsLogger:
    def __init__(self, metadata: InfiniKVEngineMetadata, log_interval: int):
        self.metadata = metadata
        self.log_interval = log_interval
        self.monitor = InfiniKVStatsMonitor.GetOrCreate()
