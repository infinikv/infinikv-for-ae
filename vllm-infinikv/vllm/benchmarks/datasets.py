...
def add_dataset_parser(parser: FlexibleArgumentParser):
    ...
    loogle_group = parser.add_argument_group("loogle dataset options")
    loogle_group.add_argument(
        "--loogle-max-input-len",
        type=int,
        default=131072,
        help="Max number of input tokens per request, used only for LooGLE dataset.",
    )
    loogle_group.add_argument(
        "--loogle-output-len",
        type=int,
        ...
        "from the LooGLE dataset.",
    )

    leval_group = parser.add_argument_group("leval dataset options")
    # TODO: 添加明确的平均输入长度
    leval_group.add_argument(
        "--leval-max-input-len",
        type=int,
        default=131072,
        help="Number of input tokens per request, used only for leval dataset.",
    )
    # TODO: 添加明确的平均输出长度
    leval_group.add_argument(
        "--leval-output-len",
...
def get_samples(args, tokenizer: TokenizerLike) -> list[SampleRequest]:
    ...
    if args.dataset_name == "custom":
        ...
        dataset = LevalDataset(dataset_path=args.dataset_path)
        input_requests = dataset.sample(
            num_requests=args.num_prompts,
            tokenizer=tokenizer,
            max_prompt_len=args.leval_max_input_len,
            output_len=args.leval_output_len,
        )

    elif args.dataset_name == "LooGLE":
        dataset = LooGLEDataset(dataset_path=args.dataset_path)
        input_requests = dataset.sample(
            num_requests=args.num_prompts,
            tokenizer=tokenizer,
            max_prompt_len=args.loogle_max_input_len,
            output_len=args.loogle_output_len,
        )
...
class LevalDataset(BenchmarkDataset):
    ...
    def load_data(
        ...
        print("Start loading LEval Dataset...")         
    ...
    def sample(
        ...
        max_prompt_len: Optional[int] = MAX_PROMPT_LEN,
        max_total_len: Optional[int] = MAX_TOTAL_LEN,
        **kwargs,
    ) -> list:
        samples: list = []
        for entry in self.data:
            
            input_prompts = entry["input"]
            # lora_request, tokenizer = self.get_random_lora_request(
            #     max_loras=max_loras, lora_path=lora_path
            # )
            for (instruction, output) in zip(entry["instructions"], entry["outputs"]):
                prompt = input_prompts + instruction
                prompt_ids = tokenizer(prompt).input_ids
                output_ids = tokenizer(output).input_ids
                prompt_len = len(prompt_ids)
                new_output_len = len(output_ids) if output_len is None else output_len
            ...
            if len(samples) >= num_requests:
                break
        print(f"len of samples: {len(samples)}, num_requests: {num_requests}")
        self.maybe_oversample_requests(samples, num_requests)
        random.shuffle(samples)
        return samples

class LooGLEDataset(BenchmarkDataset):
    """
    LooGLE Long Document QA Dataset.
    Each entry has: context, question, answer.
    ...
    def load_data(self) -> None:
        ...
        print("Start loading LooGLE Dataset...")
        pbar = tqdm.tqdm(total=num_lines)
        self.data = []
        for file in files:
            with open(file, "r") as f:
    ...
    def sample(
        ...
        **kwargs,
        ...
        print(
            f"len of samples: {len(samples)}, num_requests: {num_requests}"
        )
        self.maybe_oversample_requests(samples, num_requests)
        random.shuffle(samples)
        return samples
