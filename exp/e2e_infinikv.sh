...
STARTUP_TIMEOUT="${STARTUP_TIMEOUT:-6000}"         # seconds
HEALTH_CHECK_INTERVAL="${HEALTH_CHECK_INTERVAL:-5}" # seconds
SHUTDOWN_WAIT="${SHUTDOWN_WAIT:-10}"              # seconds
LMCACHE_CONFIG_FILE="${LMCACHE_CONFIG_FILE:-lmcache_dram_lw.yaml}"
...
for RATE in "${REQUEST_RATES[@]}"; do
    ...
    export TARDIS_CONFIG_FILE="infinikv_config_gpu1.yaml"
    ...
    sudo -S bash sosp/infinikv/csrc/GeminiFS/scripts/umount_nvme_layer_and_reset.sh
    sudo -S bash sosp/infinikv/csrc/GeminiFS/scripts/reset_snvme.sh
